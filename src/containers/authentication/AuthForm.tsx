...
// entity
import Credential, { SignUpCredential } from '../../core/lib/entities/credential';

const mapStateToProps = (state: RootState) => ({ auth: state.auth});
const mapDispatchToProps = (dispatch: Dispatch) => ({
    ...bindActionCreators({...authPresenterActions}, dispatch)
});
enum AuthInputType {
    email = 'email',
    password = 'password',
...
class AuthForm extends React.Component<Props, State> {
    ...
    componentDidUpdate(prevProps: Props) {
        const { isSignIn : prevIsSignIn } = prevProps.auth;
    ...
    onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        e.preventDefault();
        if(authType === 'signup') this.props.SIGN_UP(new SignUpCredential({ username: email, name, password }));
        else this.props.SIGN_IN(new Credential({ username: email, password}));
    };
    // common method for each authenticate situation.
    redirecter = (type: string) => {
        const { replace, location: { state } } = this.props.history;
